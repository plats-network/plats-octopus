...
fn create_campaign_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(<Test as Config>::Currency::reserved_balance(BOB),1000);
		//Check system account when clien deposit
		let sys_account_1 = Task::account_id(0);
		assert_eq!(<Test as Config>::Currency::free_balance(sys_account_1), 1000);
		...
		assert_eq!(<Test as Config>::Currency::reserved_balance(BOB),2000);	
	});
}
...
fn claim_should_be_working() {
	new_test_ext().execute_with(|| {
		...
		//Before user 1 claim
		assert_eq!(<Test as Config>::Currency::free_balance(USER1),0);
		// User 1 claim
		assert_ok!(Task::claim(Origin::root(),0, 1000, USER1));

		//after user 1 claim
		assert_eq!(<Test as Config>::Currency::free_balance(USER1),1000);

		//Remaining balance in system
		assert_eq!(<Test as Config>::Currency::free_balance(Task::account_id(0)), 4000);

		// User 2 claim
		assert_ok!(Task::claim(Origin::root(),0, 1000, USER2));

		//Remaining balance in system
		assert_eq!(<Test as Config>::Currency::free_balance(Task::account_id(0)), 3000);
